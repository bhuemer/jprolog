...
public class RobinsonUnificationStrategy implements UnificationStrategy {
    ...
    public List<UnificationPair> unify(List<UnificationPair> termsToUnify) {
        List<UnificationPair> workingSet = new ArrayList<UnificationPair>(termsToUnify);
        ...
        for (int i = 0; i < workingSet.size(); ++i) {
            ...
            Term lhs = currentPair.getLhs();
            Term rhs = currentPair.getRhs();
    ...
    private boolean unify(Term lhs, Term rhs, List<UnificationPair> workingSet, List<UnificationPair> unifier) {
        if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE,
                "    Attempting to unify the terms ''{0}'' and ''{1}''.", new Object[] { lhs, rhs });
        }

        // I'm aware of the fact that this bit of code looks like it would be appropriate to introduce
        // a separate method in the term interface that enables terms to unify themselves with other
        // terms. In doing so, I could get rid of this "if-then-else-dispatch-mess", but as I've
        // already mentioned in the documentation of the UnificationStrategy interface, there's no
        // single unification algorithm as there is no single sorting algorithm either.
        // The drawback of this approach is of course the need to adapt every single algorithm
        // implementation, if one introduces a new term. However, I can't think of any such term and
        // I think it's more likely that one wants to implement a different unification algorithm
        // (especially as the Robinson unification algorithm is not the fastest one), rather than to
        ...
        // (e.g. an IllegalStateException or OperationNotSupportedException), however, I think it's
        // not necessarily invalid to pass something else as parameter, this strategy just can't unify
        // it, that's it. 
        if (logger.isLoggable(Level.SEVERE)) {
            logger.log(Level.SEVERE,
                "    Cannot unify the terms as the kind of term ''{0}'' is unknown.", lhs);
        }
    ...
    private boolean unify(Constant constant, Term term,
        ...
        if (term instanceof Variable) {
            // If we're unifying this constant with a variable, just delegate this
            // unification call to the variable as this has already been implemented
            // below.
            return unify((Variable) term, constant, workingSet, unifier);
        }

        // Note that there is only one other case where unification succeeds if the
        ...
        // Otherwise, this left-hand side constant and the right-hand side term can't be unified.
        return false;
    }

    private boolean unify(Variable variable, Term term,
                                    List<UnificationPair> workingSet, List<UnificationPair> unifier) {
        // If we're unifying the same variable, like "X = X", unification succeeds, but
        // we don't have to anything, i.e. we basically skip this pair of variables
        ...
        if (term.occurs(variable)) {
            if (logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "    Cannot unify the variable ''{0}'' and the term " +
                        "''{1}'' as the variable occurs in the term.", new Object[] { variable, term });
            }

            return false;
        }

        // Otherwise unification succeeds, we found a new pair in our unifier and
        // now have to process it (i.e. substitute any occurrences of this variable
        // in the working set and the current state of the most general unifier).
    ...
    private boolean unify(Predicate predicate, Term term,
                                    List<UnificationPair> workingSet, List<UnificationPair> unifier) {
        if (term instanceof Variable) {
            ...
            if (!predicate.getName().equals(rhsPredicate.getName())
                    || predicate.getArgs().size() != rhsPredicate.getArgs().size()) {
                // .. unification fails.
                return false;
            }

            // Otherwise try to unify all arguments ..
            for (Iterator<Term> lhsArgs = predicate.getArgs().iterator(),
                                rhsArgs = rhsPredicate.getArgs().iterator();
                    lhsArgs.hasNext() && rhsArgs.hasNext();) {
                // .. by adding all those pairs to our working set. In doing so,
                // it will eventually be processed.
                workingSet.add(new UnificationPair(lhsArgs.next(), rhsArgs.next()));
            }
    ...
    private void substitute(Term variable, Term replacement, List<UnificationPair> list) {
        for (int i = 0; i < list.size(); i++) {
            ...
            list.set(i, new UnificationPair(
